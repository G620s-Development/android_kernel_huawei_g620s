...
static int kionix_i2c_write(struct kionix_accel_driver *acceld, u8 reg, u8 value)
{
	...
	if(loop == 0){
		...
		acceld->gsensor_test_exception.i2c_err_num = err;
		pt_kx023_dsm_ops->dump_i2c_status(acceld);
	}
	return err;
}
	...
	if(atomic_read(&acceld->accel_enabled) > 0) {
		if(atomic_read(&acceld->accel_enable_resume) > 0)
		{
			...
			if (err < 0) {
				...
				pt_kx023_dsm_ops->check_exception(acceld->accel_data[acceld->axis_map_x],
								acceld->accel_data[acceld->axis_map_y],
								acceld->accel_data[acceld->axis_map_z],acceld);
